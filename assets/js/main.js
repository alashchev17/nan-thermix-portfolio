$(document).ready(function () {
  ...
  $('a[href^="#"]').bind("click", function (e) {
    ...
    if (section !== "#") {
      let offsetTop = $(".header").height() + 32;

      const sectionOffsets = [
        { section: "#cell", offset: 90 },
        { section: "#system", offset: -300 },
      ];

      sectionOffsets.forEach(function (item) {
    ...
    if ($(window).width() > 1024) {
      ...
      function swipeSlide(direction) {
        if (!isCellSlideAnimating) {
          ...
          if (direction === "right") {
            nextSlide =
              currentSlide.next().length !== 0 ? currentSlide.next() : null;
            nextBullet =
              currentBullet.next().length !== 0 ? currentBullet.next() : null;
          } else {
            nextSlide =
              currentSlide.prev().length !== 0 ? currentSlide.prev() : null;
            nextBullet =
              currentBullet.prev().length !== 0 ? currentBullet.prev() : null;
          }
